...
var debug__default = /* @__PURE__ */ _interopDefaultCompat(debug), __defProp$6 = Object.defineProperty, __defNormalProp$6 = (obj, key, value) => key in obj ? __defProp$6(obj, key, { enumerable: !0, configurable: !0, writable: !0, value }) : obj[key] = value, __publicField$6 = (obj, key, value) => __defNormalProp$6(obj, typeof key != "symbol" ? key + "" : key, value);
...
function handleFuncCallNode(node, scope) {
  switch (`${node.namespace}.${node.name}`) {
    case "array.compact": {
      ...
      return mapConcrete(arg, scope, (arg2) => {
        if (arg2.type !== "array")
          return { type: "null" };
        const of = mapConcrete(arg2.of, scope, (of2) => of2);
        return {
          type: "array",
    ...
    case "array.join": {
      ...
      return mapConcrete(
        arrayArg,
        scope,
        (arrayArg2) => mapConcrete(sepArg, scope, (sepArg2) => arrayArg2.type !== "array" ? { type: "null" } : sepArg2.type !== "string" ? { type: "null" } : mapConcrete(arrayArg2.of, scope, (of) => of.type !== "string" && of.type !== "number" && of.type !== "boolean" ? { type: "unknown" } : { type: "string" }))
      );
    }
    case "array.unique": {
      const arg = walk({ node: node.args[0], scope });
      return mapConcrete(arg, scope, (arg2) => arg2.type !== "array" ? { type: "null" } : arg2);
    }
    case "global.lower": {
      const arg = walk({ node: node.args[0], scope });
      return mapConcrete(arg, scope, (arg2) => arg2.type === "string" ? arg2.value !== void 0 ? {
        type: "string",
    ...
    case "global.count": {
      const arg = walk({ node: node.args[0], scope });
      return mapConcrete(arg, scope, (arg2) => arg2.type === "array" ? { type: "number" } : { type: "null" });
    }
    ...
    case "global.round": {
      ...
      return mapConcrete(numNode, scope, (num) => {
        ...
        if (node.args.length === 2) {
          ...
          return mapConcrete(precisionNode, scope, (precision) => precision.type !== "number" ? { type: "null" } : { type: "number" });
        }
        return { type: "number" };
      });
    }
    case "global.string": {
      const arg = walk({ node: node.args[0], scope });
      return mapConcrete(arg, scope, (node2) => node2.type === "string" || node2.type === "number" || node2.type === "boolean" ? node2.value ? {
        type: "string",
    ...
    case "math.sum": {
      ...
      return mapConcrete(values, scope, (node2) => node2.type === "array" ? mapConcrete(node2.of, scope, (node3) => node3.type === "number" || node3.type === "null" ? { type: "number" } : { type: "null" }) : { type: "null" });
    }
    case "math.avg": {
      const values = walk({ node: node.args[0], scope });
      return mapConcrete(values, scope, (node2) => node2.type === "array" ? mapConcrete(node2.of, scope, (node3) => node3.type === "number" ? { type: "number" } : { type: "null" }) : { type: "null" });
    }
    case "math.max":
    case "math.min": {
      const values = walk({ node: node.args[0], scope });
      return mapConcrete(values, scope, (node2) => node2.type === "array" ? mapConcrete(node2.of, scope, (node3) => node3.type === "number" ? node3 : { type: "null" }) : { type: "null" });
    }
    case "pt.text":
    ...
    case "string.startsWith": {
      ...
      return mapConcrete(strTypeNode, scope, (strNode) => strNode.type !== "string" ? { type: "null" } : mapConcrete(prefixTypeNode, scope, (prefixNode) => prefixNode.type !== "string" ? { type: "null" } : { type: "boolean" }));
    }
    case "string.split": {
      const strTypeNode = walk({ node: node.args[0], scope }), sepTypeNode = walk({ node: node.args[1], scope });
      return mapConcrete(strTypeNode, scope, (strNode) => strNode.type !== "string" ? { type: "null" } : mapConcrete(sepTypeNode, scope, (sepNode) => sepNode.type !== "string" ? { type: "null" } : { type: "array", of: { type: "string" } }));
    }
    case "sanity.versionOf": {
      const typeNode = walk({ node: node.args[0], scope });
      return mapConcrete(typeNode, scope, (typeNode2) => typeNode2.type !== "string" ? { type: "null" } : { type: "array", of: { type: "string" } });
    }
    default:
      return { type: "unknown" };
  }
...
function handleObjectSplatNode(attr, scope) {
  ...
  return $trace("object.splat.value %O", value), mapConcrete(value, scope, (node) => {
    if (node.type !== "object")
      return { type: "unknown" };
    const attributes = {};
...
function handleObjectNode(node, scope) {
  ...
  for (const [idx, attr] of node.attributes.entries()) {
    ...
    if (attr.type === "ObjectConditionalSplat") {
      ...
      if ($trace("object.conditional.splat.condition %O", condition), condition === !1)
        continue;
      const attributeNode = handleObjectSplatNode(attr, scope);
      if ($trace("object.conditional.splat.result %O", attributeNode), condition === !0)
      ...
      const variant = mapConcrete(attributeNode, scope, (attributeNode2) => ($trace("object.conditional.splat.result.concrete %O", attributeNode2), attributeNode2.type !== "object" ? { type: "unknown" } : {
        type: "object",
...
function handleFilterNode(node, scope) {
  const base = walk({ node: node.base, scope });
  return $trace("filter.base %O", base), mapConcrete(base, scope, (base2) => {
...
function handleNegNode(node, scope) {
  const base = walk({ node: node.base, scope });
  return base.type !== "number" ? { type: "null" } : base.value !== void 0 ? { type: "number", value: -base.value } : base;
}
function handlePosNode(node, scope) {
  const base = walk({ node: node.base, scope });
  return base.type !== "number" ? { type: "null" } : base;
}
  ...
  if (left.type === "union" && isPrimitiveTypeNode(right))
    for (const node of left.of) {
      if (isPrimitiveTypeNode(node) && node.value === right.value)
        return !0;
      if (isPrimitiveTypeNode(node) && node.value === void 0)
        return;
    }
...
function resolveCondition(expr, scope) {
  switch ($trace("resolveCondition.expr %O", expr), expr.type) {
    ...
    case "Value": {
      ...
      return value.type === "boolean" ? value.value : value.type === "null" || value.type === "object" || value.type === "array" ? !1 : void 0;
    }
    case "And": {
    ...
    case "Not": {
      ...
      return result === void 0 ? void 0 : result === !1;
    }
    case "Group":
